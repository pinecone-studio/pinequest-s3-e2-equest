...
export default function StudentApp() {
  ...
  const [browserTestMessage, setBrowserTestMessage] = useState<string | null>(
    null,
  );
  const [isBrowserTestRunning, setIsBrowserTestRunning] = useState(false);
    ...
    startTransition(async () => {
      try {
        ...
        if (finalize) {
          ...
          setAttempt(null);
          window.location.assign("/seb/success");
          return;
        }
      } catch (err) {
        setError(err instanceof Error ? err.message : "Алдаа гарлаа.");
  ...
  const handleBrowserGraphqlTest = async () => {
    setBrowserTestMessage(null);
    setIsBrowserTestRunning(true);
    ...
    try {
      ...
      setBrowserTestMessage(
        `GraphQL хүсэлт амжилттай ажиллалаа. ${students?.length ?? 0} сурагчийн мэдээлэл ирлээ.`,
      );
    } catch (err) {
      setBrowserTestMessage(
        err instanceof Error ? err.message : "GraphQL тест амжилтгүй боллоо.",
      );
    } finally {
      setIsBrowserTestRunning(false);
    }
  };
      ...
      <div className="flex min-h-screen items-center justify-center bg-slate-950 text-white">
        <div className="rounded-xl border border-rose-500/20 bg-rose-500/10 p-8 text-center">
          ...
          {error.includes("Safe Exam Browser") ? (
            <a
              href="/api/seb/config"
              className="mt-4 inline-flex rounded-lg border border-cyan-400/40 bg-cyan-400/10 px-6 py-2 text-sm text-cyan-200"
            >
              Safe Exam Browser config татах
            </a>
          ) : null}
          <button
      ...
      <main className="mx-auto max-w-4xl space-y-8">
        <header className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <Link
              href="/api/seb/config"
              className="rounded-full border border-cyan-400/30 bg-cyan-400/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-cyan-300 transition hover:bg-cyan-400/20"
            >
              SEB Config Татах
            </Link>
          </div>
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="space-y-2">
              ...
              <h2 className="text-xl font-semibold text-white">
                Энгийн browser-оор SEB хамгаалалт шалгах
              </h2>
              <p className="max-w-2xl text-sm text-slate-400">
                `SEB_ENFORCE_CONFIG_KEY=true` үед доорх GraphQL тест нь Safe Exam
                Browser шаардах ёстой. Харин quit page товч нь submit success
                хуудас руу оруулж байгаа эсэхийг энгийн browser дээр шалгана.
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <button
                type="button"
                onClick={() => void handleBrowserGraphqlTest()}
                disabled={isBrowserTestRunning}
                className="rounded-full bg-cyan-500 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-cyan-400 disabled:opacity-50"
              >
                {isBrowserTestRunning
                  ? "GraphQL тестлэж байна..."
                  : "GraphQL Test"}
              </button>
              <Link
                href="/seb/success"
